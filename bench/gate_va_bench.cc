...
static void BM_VariantGatesVisitorEval(benchmark::State& state) {
  while (state.KeepRunning()) {
    state.PauseTiming();
    auto unif = std::uniform_real_distribution<double>(0.0, 1.0);
    auto rng = std::mt19937_64(state.range(0));
    ...
    state.ResumeTiming();
    auto z = va_gate::apply_gates_stdvisitor(bits, gs);
  }
}
BENCHMARK(BM_VariantGatesVisitorEval)
    ->Args({100000});

static void BM_VariantGatesExplicitVisitorEval(benchmark::State& state) {
  while (state.KeepRunning()) {
    state.PauseTiming();
    auto unif = std::uniform_real_distribution<double>(0.0, 1.0);
    auto rng = std::mt19937_64(state.range(0));
    ...
    state.ResumeTiming();
    auto z = va_gate::apply_gates_explicit_stdvisitor(bits, gs);
  }
}
BENCHMARK(BM_VariantGatesExplicitVisitorEval)
    ->Args({100000});
...
static void BM_BoostWhichVariantGatesEval(benchmark::State& state) {
  while (state.KeepRunning()) {
    state.PauseTiming();
    auto unif = std::uniform_real_distribution<double>(0.0, 1.0);
    auto rng = std::mt19937_64(state.range(0));
    ...
    state.ResumeTiming();
    auto z = va_gate::apply_which_gates(bits, gs);
  }
}
BENCHMARK(BM_BoostWhichVariantGatesEval)
    ->Args({100000});
